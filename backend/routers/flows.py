...
def _resolve_range(
    ...
    if range_name == "today":
        ...
        if not (from_iso and to_iso):
            raise HTTPException(
                status_code=400,
                detail="Custom range requires both 'from' and 'to' (ISO 8601).",
            )
        try:
        ...
        raise HTTPException(
            status_code=400,
            detail=f"Unknown range '{range_name}'.",
        )
...
def get_flows(
    range: Literal["today", "7d", "30d", "ytd", "custom"] = Query(
        "7d", description="Tijdframe: today, 7d, 30d, ytd, custom"
    ),
    from_: Optional[str] = Query(
        None, alias="from",
        description="ISO 8601 (alleen bij range=custom)",
    ),
    to: Optional[str] = Query(
        None, description="ISO 8601 (alleen bij range=custom)"
    ),
):
