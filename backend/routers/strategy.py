...
Vergelijkt 4 scenario's op historische data:
- A: huidig (saldering 1:1)
- B: na 2027, raw (geen saldering, export tegen overschot-prijs)
- C: na 2027 + curtailment (PV uit bij negatieve-spot uren als dat goedkoper is)
- D: na 2027 + batterij (X kWh, greedy charge/discharge)

Vaste kosten/dag (-0.18836) zijn constant en zitten NIET in de simulatie.
Response wordt 5 minuten gecached per (range, battery_kwh, from, to) tuple.
"""
...
BATTERY_ROUND_TRIP_EFF = 0.90  # 95% laden × 95% ontladen ≈ 90%
BATTERY_EFF_LEG = BATTERY_ROUND_TRIP_EFF ** 0.5  # per leg
...
class ScenarioSummary(BaseModel):
    ...
    pv_curtailed_kwh: Optional[float] = None  # alleen voor curtailment scenario
    battery_max_soc_kwh: Optional[float] = None
    savings_vs_no_saldering_eur: Optional[float] = None  # negatieve = duurder
...
def get_strategy(
    ...
    # Per-scenario daily aggregation
    daily_current = defaultdict(float)        # var cost
    ...
    for r in hourly:
        ...
        # Scenario D: batterij (greedy: surplus laden, deficit ontladen)
        net = house - pv  # positief = tekort (moet importeren), negatief = surplus
        if net < 0:
            surplus = -net
            room = battery_kwh - soc
            charge = min(surplus, room)
            soc += charge * BATTERY_EFF_LEG
            b_exp = surplus - charge  # rest exporteert tegen overschot-prijs
            b_imp = 0.0
        else:
            deficit = net
    ...
    # Daily fixed costs are constant
    ...
    def make_summary(id_, label, daily_var, **extra):
        ...
        return ScenarioSummary(
            ...
            savings_vs_no_saldering_eur=None,  # vullen we hieronder
        )
